...
export class GeneralInfoProjectEditComponent implements OnInit {
  ...
  @Input() previewImgEdit: string;
  @Input() previewImgTwoEdit: string;
  @Input() previewImgThreeEdit: string;

  /* variable to receive preview video to display on the service card and service table */
  @Input() previewVideoEdit: string;
  ...
  constructor(private sanitizer:DomSanitizer, private generalInfoService: GeneralInfoProjectService) { }

  ngOnInit(): void {
    this.generalInfoProject = this.generalInfoService.generalInfoProject;
    this.fill();
  }

  /* fill in the values of the entries to be edited */
  fill(){
    this.nameService = this.generalInfo.nameService;
    this.hashtags = this.generalInfo.hashtags;
    this.category = this.generalInfo.category;
    this.subcategory = this.generalInfo.subcategory;
    this.videoFile = this.generalInfo.video;
    this.imageFile = this.generalInfo.images;
    this.filesProject = this.generalInfo.files;

    this.previewImg = this.previewImgEdit;
    this.previewImgTwo = this.previewImgTwoEdit;
    this.previewImgThree = this.previewImgThreeEdit;
    this.url = this.previewVideoEdit;
  }

  /* delete  to add edited information */
  deleteGeneralInfo(generalInfo:generalInfoProject){
    this.generalInfoService.delete(generalInfo);
  }
